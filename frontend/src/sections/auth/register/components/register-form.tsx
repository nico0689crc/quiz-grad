...
import { IconButton, InputAdornment, Stack, Typography, Link, TextField, useMediaQuery } from '@mui/material';
...
export default function RegisterForm() {
  ...
  const onSubmitHandler = useCallback(
    handleSubmit(async ({ email, password, passwordConfirmation, firstName, lastName }) => {
      try {
        await registerUser(email, password, passwordConfirmation, firstName, lastName);
  ...
  return (
    <Stack component='form' rowGap={3} onSubmit={onSubmitHandler}>
      ...
      <Stack direction='row' spacing={0.5}>
        <Typography variant='body2'>{t('register.labels.have_account')}</Typography>
