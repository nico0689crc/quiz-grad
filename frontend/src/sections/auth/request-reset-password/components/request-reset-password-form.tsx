...
export default function RequestResetPasswordForm() {
  ...
  const onSubmitHandler = useCallback(
    handleSubmit(async ({ email }) => {
      try {
        ...
        enqueueSnackbar(t('request_reset_password.messages.success'), {
          variant: 'success',
          anchorOrigin: isUpMd
  ...
  return (
    <Stack component='form' rowGap={3} onSubmit={onSubmitHandler}>
      <Typography variant='h4'>{t('request_reset_password.labels.title')}</Typography>

      <Typography variant='body2' sx={{ color: 'text.secondary' }}>
        {t('request_reset_password.labels.sub_title')}
      </Typography>

      <TextField
        {...register('email')}
        label={t('request_reset_password.labels.email')}
      ...
      <LoadingButton
        ...
        type='submit'
        color='primary'
        label={t('request_reset_password.labels.send')}
        loadingLabel={t('request_reset_password.labels.sending')}
      />
