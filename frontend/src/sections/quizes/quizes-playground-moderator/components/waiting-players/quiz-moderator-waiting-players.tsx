import { Container, Typography, Card, Stack, Divider, Box, Button, useMediaQuery } from '@mui/material';
...
const QuizModeratorWaitingPlayers = () => {
  ...
  return (
    <Container maxWidth='lg' sx={{ display: 'grid', gridTemplateRows: '100%' }}>
      <Box component={Card} sx={{ display: 'grid', gridTemplateRows: 'auto auto auto' }}>
        <Box
          sx={{
            display: 'grid',
        ...
        >
          <Stack sx={{ alignItems: 'center', justifyContent: 'space-evenly' }}>
            <Typography variant='subtitle1'>{t('waiting_for_players.labels.waiting_for_players')}</Typography>
            <Image src='/assets/illustrations/waiting_for_players.svg' height={0.4} />
          ...
          <Stack spacing={2} py={3} sx={{ alignItems: 'center', justifyContent: 'space-evenly' }}>
            ...
            <Divider sx={{ width: '50%' }} variant='middle'>
              <Typography variant='subtitle2'>{t('common.labels.or')}</Typography>
            </Divider>
        ...
        <Divider sx={{ width: '100%' }}>
          <Typography variant='subtitle2'>{t('common.labels.players')}</Typography>
        </Divider>

        {players.length === 0 ? (
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 3 }}>
            <Typography variant='subtitle1'>{t('waiting_for_players.labels.no_players')}</Typography>
          </Box>
        ) : (
          <Box
            sx={{
