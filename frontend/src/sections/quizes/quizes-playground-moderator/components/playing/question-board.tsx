...
const QuestionBoard = () => {
  ...
  const { quiz, showNextQuestionButton, isLastQuestion, questionsTotal } = useAppSelector(
    (state: RootState) => state.room.room
  );
  const { sendNextQuestion, isRunning, totalSeconds } = useQuizModeratorContext();
  ...
  return (
    <Stack spacing={3}>
      {quiz && (
        ...
        >
          <Stack direction='row' alignItems={{ xs: 'flex-start', lg: 'center' }} spacing={1}>
            <Typography variant='h5' sx={{ color: (theme) => theme.palette.primary.main, textDecoration: 'underline' }}>
              Quiz:
            </Typography>
          ...
          {isRunning && (
            <Stack direction='row' spacing={1}>
              <Typography
                variant='h5'
                sx={{ color: (theme) => theme.palette.primary.main, textDecoration: 'underline' }}
              >
      ...
      {question && <QuizQuestionItem question={question} showCollapse={false} actions={actionsContent} />}
    </Stack>
  );
};
